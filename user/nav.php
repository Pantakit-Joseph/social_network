<?php
$sql_rq = "SELECT * FROM `users` WHERE `user_id` IN (
    SELECT `friends`.`user_id` FROM `friends` WHERE `friends`.`friend_id`='$user_id' AND `friends`.`accept`=0
						...
						<ul class="dropdown-menu dropdown-menu-end">
							...
							<li class="px-3 py-2">
								<div class="d-flex align-items-center gap-2" style="width: 400px;">
									...
									<a href="friend.php?i=1&uid=<?= $rq['user_id'] ?>&fid=<?= $user_id ?>" class="btn btn-sm btn-warning">อนุมัติ</a>
									<a href="friend.php?i=0&uid=<?= $rq['user_id'] ?>&fid=<?= $user_id ?>" class="btn btn-sm btn-danger">ปฏิเสธ</a>
								</div>
							</li>
							<?php } ?>
						</ul>
